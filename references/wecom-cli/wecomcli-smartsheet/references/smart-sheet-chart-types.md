...
| `filter` | FilterSpec | 筛选条件，不传则不过滤；禁止传 `"filter": {}` 或空的 `conditions`。必须查看 `references/smart-sheet-view-types.md` 中的 FilterSpec 定义 |
...
>
> **强制流程**：带筛选条件的图表创建前，**必须先调用 `smartsheet fields list` 获取字段的 `property_single_select.options[]` / `property_select.options[]`**，从中取 `id` 填入 `string_value.value`。完整的 FilterSpec / StringValue 定义见 `references/smart-sheet-view-types.md`。
