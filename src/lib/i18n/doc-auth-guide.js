...
function renderNeedsAuthGuide(locale, { docConfig }) {
  if (locale === 'en-US') {
    return [
      'The bot has not completed WeCom document authorization yet.',
      '',
      'Before the first smart-sheet / WeCom doc request, guide the user through these steps:',
      '1. Open the document authorization page for the current bot',
      renderAuthPageLine(locale, { docConfig }),
      '2. In WeCom, confirm that document capability is authorized for the current bot',
      '3. After authorization completes, return to the conversation and send the original doc / smart-sheet request again',
      '4. Then run mcporter list wecom-doc --output json / mcporter call wecom-doc.<tool>',
      '',
      'If the user has not finished authorization, common follow-up symptoms are:',
  ...
  return [
    '当前机器人还没有完成企业微信文档授权。',
    '',
    '首次使用智能表格 / 企微文档时，先引导用户完成以下步骤：',
    '1. 打开当前机器人的文档授权页',
    renderAuthPageLine(locale, { docConfig }),
    '2. 在企业微信里确认把文档能力授权给当前机器人',
    '3. 授权完成后，回到对话重新发起原来的文档/智能表格请求',
    '4. 此时再执行 mcporter list wecom-doc --output json / mcporter call wecom-doc.<tool>',
    '',
    '如果用户没有走完授权，后续常见现象是：',
    '- 有 MCP URL，但调用仍失败',
    '- 返回 850001 或类似授权不足错误',
